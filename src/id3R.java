...
/**
 * Created by Nisansa on 15/01/23.
 */
...
public class id3R {
    ...
    public static void main(String[] args) {
        if (args.length != 4) {
            ...
            id3R dt = new id3R("training_set.csv", "test_set.csv","validation_set.csv", "model.csv");
            System.err.println("Error in arguments");
        } else {
            ...
            id3R dt = new id3R(args[0], args[1], args[2],args[3]); //"training_set", "test_set","validation_set", "model"
        }
    }

    public id3R(String trainingSet,String testSet,String validationSet,String modelFile) {
        ...
        String[][] val_data = readFile(validationSet);
        EntrySet val_set = new EntrySet(val_data);
        ...
        //Prune
        Iterator<HashMap<String, Boolean>> itr = val_set.getIterator();
        int count = 0;
        double accuracy = 100;
        while (itr.hasNext()) {
        ...
        accuracy = ((((double) count * 100) / (double) val_set.Size()));
        System.out.println("\nOriginal Accuracy : " + accuracy + "%");

        double newAccuracy = accuracy;
        ArrayList<Node> leaves=n.getLeafList();
        int pruneCount=0;
        do {
            pruneCount++;
            leaves=n.getLeafList();
            for (int i = 0; i <leaves.size() ; i++) {
                leaves.get(i).parent.TutnNodeOff();
            }

            itr = val_set.getIterator();
            count = 0;
            while (itr.hasNext()) {
                HashMap<String, Boolean> observation = itr.next();
                if (n.evaluate(observation) == observation.get("Class")) {
            ...
            newAccuracy = ((((double) count * 100) / (double) val_set.Size()));
            System.out.println("New Accuracy after pruning "+pruneCount+" levels : " + newAccuracy + "%");
        }while(newAccuracy>accuracy);


        for (int i = 0; i <leaves.size() ; i++) {  //We need to turn the last level on again (Because turning it off reduced accuracy)
            leaves.get(i).parent.TutnNodeON();
        }
        ...
        accuracy = ((((double) count * 100) / (double) val_set.Size()));
        System.out.println("Restored Accuracy : " + accuracy + "%\n");



        //Test
        itr = test_set.getIterator();
        count = 0;
        accuracy = 100;
        while (itr.hasNext()) {
            HashMap<String, Boolean> observation = itr.next();
...
/*		//Add randomization
        ...
        public ArrayList<Node> getLeafList(){
            ...
            if(winner==null){
                ...
                leaves.addAll(pChild.getLeafList());
                leaves.addAll(nChild.getLeafList());
            }
            return leaves;
        }

        public void TutnNodeOff(){
            if(winner!=null) {
                winnerBackup = new GainElement(winner);
                winner = null;
            }
        }

        public void TutnNodeON(){
            if(winner==null && winnerBackup!=null) {
                winner = new GainElement(winnerBackup);
            }
        }
    ...
    public class GainElement implements Comparable {
        ...
        public GainElement(GainElement g) {
            this.attribute = g.attribute;
            this.gain = g.gain;
            this.phalf = g.phalf;
            this.nhalf = g.nhalf;
            this.remainder= g.remainder;
        }
